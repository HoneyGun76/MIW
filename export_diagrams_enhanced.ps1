...
# Exports all main diagrams to JPG format using PlantUML JAR
    ...
    if (Test-Path $sourcePath) {
        ...
        try {
            ...
            if (Test-Path $expectedOutput) {
                Write-Host "  ✓ Successfully exported: $baseName.png" -ForegroundColor Green
                $successCount++
            } else {
                Write-Host "  ✗ Export failed: $diagram" -ForegroundColor Red
            }
        }
        catch {
...
if ($successCount -gt 0) {
    ...
    Write-Host ""
    Write-Host "No diagrams were exported successfully. Please check the errors above." -ForegroundColor Red
}
}
